...
class DilatedAttentionNew(nn.Module):
    ...
    def forward(self, x):
        ...
        max_subatt_n, sparse_indices, padding_mask = SparsifyIndices(
            x, [self.segment_size], [self.dilation_rate], head_idx
        )
        ...
        x = x.gather(
            2,
            sparse_indices[:, :x_dim1, :]
            .unsqueeze(-1)
            .expand(-1, -1, -1, self.d_model),
        )
        ...
        print(
            f"attn output shape and type: {attn_output.shape} and dtype: {attn_output.dtype}"
        )
        ...
        if self.use_rel_pos_bias:
            ...
            print(
                f"attn_output shape and dtype: {attn_output.shape} and dtype: {attn_output.dtype}"
            )
        ...
        # Mix outputs
        attn_output = MixOutputs(
            (batch_size, seq_len, self.d_model),
            x.dtype,
            x.device,
            attn_output,
            attn_output.sum(dim=-1),
            sparse_indices,
        )
